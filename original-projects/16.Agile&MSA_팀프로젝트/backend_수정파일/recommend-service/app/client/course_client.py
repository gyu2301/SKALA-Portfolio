...
class CourseServiceClient:
    ...
    async def get_recommend_courses(
        ...
        카테고리 기반 미예약 상품 목록 조회 (예약생 수 기준 정렬)
        """
        ...
        try:
            ...
            logger.error(f"[CourseClient] 추천 상품 조회 실패 - category: {category}, error: {e}")
            return []

    async def get_all_courses(self) -> List[CourseResponse]:
        """
        GET /courses - 전체 상품 목록 조회
        ...
        try:
            ...
            logger.error(f"[CourseClient] 전체 상품 조회 실패 - error: {e}")
            return []
