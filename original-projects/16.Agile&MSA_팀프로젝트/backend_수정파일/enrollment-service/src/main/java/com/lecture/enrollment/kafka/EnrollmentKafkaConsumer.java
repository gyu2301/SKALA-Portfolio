...
public class EnrollmentKafkaConsumer {
    ...
    public void handlePaymentCompleted(Map<String, Object> event) {
        ...
        try {
            ...
            // #14 payload 확장 — eventId 없는 옛 포맷도 그대로 동작해야 한다 (null 허용)
            Object eventIdValue = event.get("eventId");
            String eventId = eventIdValue != null ? eventIdValue.toString() : null;

            log.info("[Kafka Consumer] payment.completed 파싱 완료 - userId: {}, courseId: {}, eventId: {}",
                    userId, courseId, eventId);

            enrollmentService.activateEnrollment(userId, courseId, eventId);

            log.info("[Kafka Consumer] 예약 확정 완료 - userId: {}, courseId: {}",
                    userId, courseId);

        } catch (Exception e) {
            log.error("[Kafka Consumer] 예약 확정 실패 - event: {}, error: {}",
