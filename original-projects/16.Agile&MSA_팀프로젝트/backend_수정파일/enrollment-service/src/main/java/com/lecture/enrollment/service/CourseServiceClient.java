...
public class CourseServiceClient {
    ...
    /**
     * Course Service: 상품 존재 여부 확인 (동기 REST)
     */
    public boolean existsCourse(Long courseId) {
        try {
            ...
            log.error("[CourseServiceClient] 상품 존재 확인 실패 - courseId: {}, error: {}",
    ...
    /**
     * Course Service: 상품 상세 조회
     * - 내 예약 목록 응답에 course 정보를 붙일 때 사용
    ...
    public Map<String, Object> getCourse(Long courseId) {
        try {
            ...
            log.info("[CourseServiceClient] 상품 상세 조회 성공 - courseId: {}", courseId);
            log.debug("[CourseServiceClient] 상품 상세 응답 - courseId: {}, body: {}", courseId, responseBody);
            ...
            log.error("[CourseServiceClient] 상품 상세 조회 실패 - courseId: {}, error: {}",
                    courseId, e.getMessage());
            throw new RuntimeException("Course Service 상품 상세 조회 실패");
        }
    }

    /**
     * Course Service: 예약 수 증가 (예약 확정 시 호출)
     */
    public void increaseEnrollmentCount(Long courseId) {
        try {
            ...
            log.info("[CourseServiceClient] 예약 수 증가 완료 - courseId: {}", courseId);
        } catch (Exception e) {
            log.error("[CourseServiceClient] 예약 수 증가 실패 - courseId: {}, error: {}",
                    courseId, e.getMessage());
        }
    }
}
